...
import { uploadBookProjectImages, deleteClassProjectImages, deleteUserProjectImages, preserveCopiedClassImages } from "./bookProjectStorage";
import { purgeClassData, purgeStudentData } from "./dataDeletion.mjs";
import { createDeletionAdapter } from "./firestoreDeletion.mjs";
import {
  collection,
  addDoc,
  setDoc,
...
export async function deleteClass(classId) {
  if (isFirebaseConfigured) {
    const adapter = createDeletionAdapter(db, {
      deleteClassImages: async (id) => {
        await preserveCopiedClassImages(adapter, id, getCurrentUser()?.uid);
        await deleteClassProjectImages(id);
      },
      deleteUserImages: deleteUserProjectImages,
    });
    await purgeClassData(adapter, classId);
    return;
  }
  ...
  const layouts = (mock.seatLayouts ?? []).filter((row) => row.classId === classId);
  const noteUids = (mock.studentNotes ?? []).filter((row) => row.classId === classId).map((row) => row.studentUid);
  for (const key of ["attendanceRecords", "seatLayouts", "groupAssignments", "studentNotes"]) {
    mock[key] = (mock[key] ?? []).filter((row) => row.classId !== classId);
  }
  for (const key of ["presence", "questionSignals"]) {
    for (const [id, row] of Object.entries(mock[key] ?? {})) if (row.classId === classId) delete mock[key][id];
  }
  ...
  layouts.forEach((row) => notifySeatLayout(classId, row.id));
  notifyGroupAssignment(classId);
  notifyQuestionSignals(classId);
  notifyPresence(classId);
  notifyBroadcast(classId);
  noteUids.forEach(notifyStudentNotes);
  notifyBookProjects();
  notifyBookActivities();
  activityIds.forEach(notifyBookEntries);
  notifyClasses();
  notifyStudyBoards();
...
const _mockDirectoryListeners = new Set();
...
export function subscribeUserDirectory(callback) {
  ...
  _mockDirectoryListeners.add(emit);
  emit();
  return () => _mockDirectoryListeners.delete(emit);
}
...
export async function deleteStudent(uid) {
  if (isFirebaseConfigured) {
    await purgeStudentData(createDeletionAdapter(db, {
      deleteClassImages: deleteClassProjectImages,
      deleteUserImages: deleteUserProjectImages,
    }), uid);
    return;
  }
  ...
  setDirectory(buildMockDirectory());
  _mockDirectoryListeners.forEach((emit) => emit());
  notifyQuestions();
