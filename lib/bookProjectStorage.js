...
function managedImageRef(value) {
  if (typeof value !== "string" || !value.startsWith("https://firebasestorage.googleapis.com/")) return null;
  try {
    const imageRef = ref(storage, value);
    return imageRef.bucket === ref(storage).bucket
      && /^book-project-images\/[^/]+\/[^/]+\/[a-f0-9]{64}\.jpg$/.test(imageRef.fullPath) ? imageRef : null;
  } catch { return null; }
}

export async function copyManagedProjectImage(value, classId, uid) {
  const source = managedImageRef(value);
  if (!source || source.fullPath.split("/")[1] === classId) return value;
  const filename = source.fullPath.split("/").at(-1);
  const target = ref(storage, `${STORAGE_PREFIX}/${assertPathSegment(classId, "수업")}/${assertPathSegment(uid, "사용자")}/${filename}`);
  try { return await getDownloadURL(target); }
  catch (error) { if (error?.code !== "storage/object-not-found") throw error; }
  const bytes = await getBytes(source, 600000);
  try {
    await uploadBytes(target, bytes, { contentType: "image/jpeg" });
  } catch (uploadCause) {
    try { return await getDownloadURL(target); }
    catch { throw uploadCause; }
  }
  return getDownloadURL(target);
}

export async function preserveCopiedClassImages(adapter, sourceClassId, uid) {
  const cache = new Map();
  async function copyValue(value, destination) {
    if (typeof value === "string") {
      const source = managedImageRef(value);
      if (!source || source.fullPath.split("/")[1] !== sourceClassId) return value;
      const key = `${destination}:${value}`;
      if (!cache.has(key)) cache.set(key, copyManagedProjectImage(value, destination, uid));
      return cache.get(key);
    }
    if (Array.isArray(value)) return Promise.all(value.map((item) => copyValue(item, destination)));
    if (value && Object.getPrototypeOf(value) === Object.prototype) {
      const entries = await Promise.all(Object.entries(value).map(async ([key, item]) => [key, await copyValue(item, destination)]));
      return Object.fromEntries(entries);
    }
    return value;
  }
  for (const collectionName of ["bookProjects", "bookActivities", "bookResources"]) {
    for (const item of await adapter.list(collectionName)) {
      const destination = item.data.classId;
      if (!destination || destination === sourceClassId) continue;
      const replacement = await copyValue(item.data, destination);
      if (JSON.stringify(replacement) !== JSON.stringify(item.data)) await adapter.update(item.path, replacement);
    }
  }
}

async function deleteImageTree(path) {
  if (!storage) throw new Error("이미지 저장소가 준비되지 않았습니다.");
  let pageToken;
  do {
    const page = await list(ref(storage, path), { maxResults: 1000, ...(pageToken ? { pageToken } : {}) });
    for (const prefix of page.prefixes) await deleteImageTree(prefix.fullPath);
...
export async function deleteClassProjectImages(classId) {
  await deleteImageTree(`${STORAGE_PREFIX}/${assertPathSegment(classId, "수업")}`);
}

export async function deleteUserProjectImages(uid) {
  const ownerUid = assertPathSegment(uid, "사용자");
  if (!storage) throw new Error("이미지 저장소가 준비되지 않았습니다.");
  let pageToken;
  do {
    const page = await list(ref(storage, STORAGE_PREFIX), { maxResults: 1000, ...(pageToken ? { pageToken } : {}) });
    for (const classroom of page.prefixes) await deleteImageTree(`${classroom.fullPath}/${ownerUid}`);
    pageToken = page.nextPageToken;
  } while (pageToken);
}
...
async function uploadBookProjectImage(user, classId, image, cache) {
  const isRemote = image.startsWith("https://");
  if (!isRemote && !image.startsWith(INLINE_JPEG_PREFIX)) {
    throw uploadError("JPG 형식의 첨부 이미지만 저장할 수 있어요.");
  }
  if (!isRemote && (!isFirebaseConfigured || !storage)) {
    throw uploadError("이미지 저장소가 준비되지 않았어요. 잠시 후 다시 저장해 주세요.");
  }
  if (cache.has(image)) return cache.get(image);
  const pendingUpload = (async () => {
    if (isRemote) return copyManagedProjectImage(image, classId, user.uid);
